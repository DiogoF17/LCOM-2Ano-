...
extern int hook_id;       //contem o identificador proveniente do hook_id
...
int (timer_set_frequency)(uint8_t timer, uint32_t freq) {
  ...
  if(timer == 0){
    timer_get_conf(TIMER_0, &st);  //obtem a configuracao do timer
    ...
    sys_outb(TIMER_0, res);           //escreve no timer os 8 bits menos significativos
    util_get_MSB(valor, &res);        
    sys_outb(TIMER_0, res);   //escreve no timer os 8 bits mais significativos
    return 0;
  }
  else if(timer == 1){
    timer_get_conf(TIMER_1, &st);  //obtem a configuracao do timer
    temp = 0x0000000F & st;      //mantem os 4 bits menos significativos
    temp = temp | 0x00000070;     //escreve os 4 bits mais significaticos
    sys_outb(TIMER_CTRL, temp);
    util_get_LSB(valor, &res);
    sys_outb(TIMER_1, res);           //escreve no timer os 8 bits menos significativos
    util_get_MSB(valor, &res);        
    sys_outb(TIMER_1, res);   //escreve no timer os 8 bits mais significativos
    return 0;
  }
  else{
